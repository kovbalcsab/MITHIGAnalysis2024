...
int main(int argc, char *argv[])
{
  ...
  TH1D *h_unweighted_EvtMult 	= (TH1D*) MultWH.h_num_norm->Clone("h_unweighted_EvtMult");
  TH1D *h_weighted_EvtMult    = (TH1D*) MultWH.h_num_norm->Clone("h_weighted_EvtMult");
  TH1D *h_target_EvtMult 		  = (TH1D*) MultWH.h_num_norm->Clone("h_target_EvtMult");
  h_unweighted_EvtMult->Reset();
  h_weighted_EvtMult->Reset();
  h_target_EvtMult->Reset();
  h_unweighted_EvtMult->GetXaxis()->SetTitle("Charged hadron multiplicity");
  h_weighted_EvtMult->GetXaxis()->SetTitle("Charged hadron multiplicity");
  h_target_EvtMult->GetXaxis()->SetTitle("Charged hadron multiplicity");
  ...
  for (Long64_t i = 0; i < nEntries; i++)
  {
    ...
    h_weighted_EvtMult    ->Fill(nTrackInAcceptanceHP, GptGyWeight*MultWeight);

    for (int iD = 0; iD < Dpt->size(); ++iD)
    {
      h_unweighted_Dpt    ->Fill(Dpt->at(iD), GptGyWeight);
      h_weighted_Dpt      ->Fill(Dpt->at(iD), GptGyWeight*MultWeight);
      h_unweighted_Dy     ->Fill(Dy->at(iD),  GptGyWeight);
      h_weighted_Dy       ->Fill(Dy->at(iD),  GptGyWeight*MultWeight);
      h_unweighted_DCA    ->Fill(DCA->at(iD), GptGyWeight);
      h_weighted_DCA      ->Fill(DCA->at(iD), GptGyWeight*MultWeight);
    }
  }
  ...
  h_unweighted_EvtMult->Scale(1/h_unweighted_EvtMult->Integral());
  h_weighted_EvtMult->Scale(1/h_weighted_EvtMult->Integral());
  h_target_EvtMult->Scale(1/h_target_EvtMult->Integral());
  ...
  normalizeHistoBinWidth(h_unweighted_EvtMult);
  normalizeHistoBinWidth(h_weighted_EvtMult);
  normalizeHistoBinWidth(h_target_EvtMult);
  ...
  )
  {
    ...
    TH1D *h_bf_closure     = (TH1D*) h_target->Clone("h_bf_closure");
    h_bf_closure->Reset();

    // Compute ratio of h_weighted_EvtMult to h_num_norm
    h_bf_closure->Divide(h_unweighted, h_target, 1.0, 1.0, "B");
    ...
    leg->AddEntry(h_target, "Data (Target)", "l");
    formatLegend(leg);
    leg->Draw();
    ...
    h_bf_closure->GetYaxis()->SetNdivisions(505);
    h_bf_closure->GetYaxis()->SetLabelSize(0.1);
    h_bf_closure->GetXaxis()->SetLabelSize(0.1);
    h_bf_closure->GetXaxis()->SetTitleSize(0.1);
    h_bf_closure->GetXaxis()->SetTitleOffset(1.3);
    h_bf_closure->SetLineColor(kBlue);
    h_bf_closure->SetMinimum(0.5);
    h_bf_closure->SetMaximum(1.5);
    h_bf_closure->Draw("ple same");

...
int main(int argc, char **argv)
{
	...
	const std::string inputFileName = CL.Get("InputFile", "fit_result.root");
	const std::string outputFileName = CL.Get("OutputFile", "iter_chi2.root");
	                                ...
	                                bool zoomToLowX) -> TCanvas *
	{
		...
		TLegend *absLegend = new TLegend(0.60, 0.55, 0.88, 0.88);
		...
		for(size_t i = 0; i < ratioLocal.size(); ++i)
		{
			...
			absLegend->AddEntry(absHist, (legendPrefix + " iter " + std::to_string(storedIterations[i])).c_str(), "l");

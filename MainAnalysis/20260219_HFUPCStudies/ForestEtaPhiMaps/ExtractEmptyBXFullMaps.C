...
double GetMaxEnergyHF(PFTreeMessenger *M, double etaMin, double etaMax, double phiMin, double phiMax, double ptMin = 0.);
...
int main(int argc, char *argv[]) {
    ...
    for (size_t iEta = 0; iEta < etaBorders.size() - 1; iEta++) {
        ...
        for (size_t iPhi = 0; iPhi < phiBorders.size() - 1; iPhi++) {
            hHFEMaxPlusPhiBins.push_back(new TH1D(Form("hHFEMaxPlus_eta%.1f_%.1f_phi%.1f_%.1f", etaBorders[iEta], etaBorders[iEta+1], phiBorders[iPhi], phiBorders[iPhi+1]), "", 300, 0, 100));
            hHFEMaxMinusPhiBins.push_back(new TH1D(Form("hHFEMaxMinus_eta%.1f_%.1f_phi%.1f_%.1f", -etaBorders[iEta+1], -etaBorders[iEta], phiBorders[iPhi], phiBorders[iPhi+1]), "", 300, 0, 100));
        }
    ...
    for (int iEntry = 0; iEntry < EntryCount; iEntry++) {
        ...
        if ((nTrkFilter == 1 && nTrackInAcceptanceHP != 0) ||
            (nTrkFilter == -1 && nTrackInAcceptanceHP == 0)) {
            continue; // Track filter selection
        }
        hNumberOfEventsAfterCuts->Fill(2); // Track filter passed

        if (MZDC.sumPlus > ZDCPlus1nThreshold || MZDC.sumMinus > ZDCMinus1nThreshold) continue; // ZDC energy cut
        ...
        for (size_t iEta = 0; iEta < etaBorders.size() - 1; iEta++) {
            for (size_t iPhi = 0; iPhi < phiBorders.size() - 1; iPhi++) {
                double HFEMaxPlus = GetMaxEnergyHF(&MPF, etaBorders[iEta], etaBorders[iEta+1], phiBorders[iPhi], phiBorders[iPhi+1], 0.0);
                double HFEMaxMinus = GetMaxEnergyHF(&MPF, -etaBorders[iEta+1], -etaBorders[iEta], phiBorders[iPhi], phiBorders[iPhi+1], 0.0);
...
double GetMaxEnergyHF(PFTreeMessenger *M, double etaMin, double etaMax, double phiMin, double phiMax, double ptMin) {
  ...
  for (int iPF = 0; iPF < M->ID->size(); iPF++) {
    ...
    if ((M->ID->at(iPF) == 6 || M->ID->at(iPF) == 7) && M->Eta->at(iPF) > etaMin && M->Eta->at(iPF) < etaMax && M->Phi->at(iPF) > phiMin && M->Phi->at(iPF) < phiMax) {
      if (M->E->at(iPF) > EMax)
        EMax = M->E->at(iPF);
    }
  }

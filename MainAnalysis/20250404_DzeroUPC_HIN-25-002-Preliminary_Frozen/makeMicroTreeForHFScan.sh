...
jq -c '.MicroTrees[]' $SampleSettingCard | while read MicroTree; do
	...
	mkdir -p $MicroTreeDir/pt${MinDzeroPT}-${MaxDzeroPT}_y${MinDzeroY}-${MaxDzeroY}_IsGammaN${IsGammaN}_DoSystRapGap${DoSystRapGap}/
	Output=$MicroTreeDir/pt${MinDzeroPT}-${MaxDzeroPT}_y${MinDzeroY}-${MaxDzeroY}_IsGammaN${IsGammaN}_DoSystRapGap${DoSystRapGap}/${MicroTreeBaseName}

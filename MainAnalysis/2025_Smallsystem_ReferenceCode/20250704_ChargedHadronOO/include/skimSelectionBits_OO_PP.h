bool checkHFANDCondition(const ChargedHadronRAATreeMessenger *ch, float HFE_min1, float HFE_min2, bool useOnlineHFE) {
...
bool checkHFORCondition(const ChargedHadronRAATreeMessenger *ch, float HFE_min, bool useOnlineHFE) {
    if (useOnlineHFE) {
        int ohiHF = max(ch->mMaxL1HFAdcPlus, ch->mMaxL1HFAdcMinus);
        if (ohiHF <= HFE_min) {
            return false;

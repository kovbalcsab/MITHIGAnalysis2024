...
public:
  ...
  // EEC histograms
  TH1D* hEECInclusive; // inclusive EEC in log bins
  TH1D* hEEC; // EEC in the double log style 
  TH1D* hTrackPt; // track pT
  TH1D* hTrackEta; // track eta
  TH1D* hHardScale; // histogram for the hard scale (whatever you set it to be)
  TH1D* hMult; // multi
  ...
  void analyze(Parameters &par) {
    ...
    hHardScale       = new TH1D("hHardScale", "", 20, par.MinHardScale, par.MaxHardScale); 
    ...
    for (unsigned long i = 0; i < nEntry; i++) {
      ...
      if ((par.IsData && triggerSelection(MDzeroJetUPC, par)) || !par.IsData) {
         ...
         for (unsigned long a = 0; a < MDzeroJetUPC->PT->size(); a++) {
            ...
            if(!((MDzeroJetUPC->isGammaN && MDzeroJetUPC->PT->at(a) > 15 && MDzeroJetUPC->Eta->at(a) > 0) || (!MDzeroJetUPC->isGammaN && MDzeroJetUPC->PT->at(a) > 15 && MDzeroJetUPC->Eta->at(a) < 0)))foundTrack = true;
         } // end first loop over the number of tracks

         
         // temp check if there is no high pt track on the photon going side skip 

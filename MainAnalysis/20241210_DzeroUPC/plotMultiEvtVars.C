...
void plotHistograms(std::vector<TH1D*> &hists, std::vector<Color_t> &colors, std::vector<std::string> &labels, std::string xTitle, std::string yTitle, std::string plotTitle, std::string outputFileName, 
    double MinDzeroPT, double MaxDzeroPT, double MinDzeroY, double MaxDzeroY, bool IsGammaN,
    ...
    latex.SetNDC();
    latex.DrawLatex(0.15, 0.85, Form("%0.0f < D^{0} p_{T} < %0.0f GeV/c", MinDzeroPT, MaxDzeroPT));
    latex.DrawLatex(0.15, 0.80, Form("%0.2f < D^{0} y < %0.2f", MinDzeroY, MaxDzeroY));
    latex.DrawLatex(0.15, 0.75, Form("%s", IsGammaN ? "Gamma-N" : "N-Gamma"));
...
int main(int argc, char** argv) {
    ...
    std::vector<std::string> histNames = CL.GetStringVector("HistNames", "");
    ...
    double MinDzeroY = CL.GetDouble("MinDzeroY", -1);
    double MaxDzeroY = CL.GetDouble("MaxDzeroY", 1);
    ...
    for (int iFile = 0; iFile < inputFileNames.size(); ++iFile) {
        ...
        TH1D* histCurr = dynamic_cast<TH1D*>(inFile->Get(histNames[iFile].c_str()));
        if (histCurr) hists.push_back((TH1D*)histCurr->Clone(Form("hist_%s", inFile->GetName())));

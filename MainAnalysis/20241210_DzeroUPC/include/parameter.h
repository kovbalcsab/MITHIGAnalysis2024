...
class Parameters {
public:
  Parameters(float MinDzeroPT, float MaxDzeroPT, float MinDzeroY, float MaxDzeroY, bool IsGammaN, int TriggerChoice,
             int BkgFilterChoice, bool IsData, int Year = 0, float scaleFactor = 1.0, int in_DoSystRapGap = 0,
             int in_DoSystD = 0, bool in_DoGptGyReweighting = false, string in_GptGyWeightFileName = "",
             bool in_DoMultReweighting = false, string in_MultWeightFileName = "", int doCCFsyst = 0,
             float in_HFMaxRapDefn = -1, bool in_DoTrkPtErrFilter = true, bool in_DoTrkHitFilter = true)
      : MinDzeroPT(MinDzeroPT), MaxDzeroPT(MaxDzeroPT), MinDzeroY(MinDzeroY), MaxDzeroY(MaxDzeroY), IsGammaN(IsGammaN),
        TriggerChoice(TriggerChoice), BkgFilterChoice(BkgFilterChoice), IsData(IsData), Year(Year),
    ...
    // Check DoSystD
    if (in_DoSystD != 0 && in_DoSystD != 1 && in_DoSystD != 2 && in_DoSystD != 3 && in_DoSystD != 4) {
      printf("[Error] Couldn't recognize the option DoSystD=%d (should be 0 = nominal, 1 = Dsvpv variation, 2: DtrkPt "
             "variation, 3: Dalpha variation, 4: Dchi2cl variation). Exiting...\n",
             in_DoSystD);
      exit(1);
    } else {
      DoSystD = in_DoSystD;
    ...
    if (doCCFsyst != 0 && doCCFsyst != 1 && doCCFsyst != 2) {
      printf("[Error] Couldn't recognize the option DoCCFsyst=%d (should be 0 = nominal, 1 = select events failing "
             "CCF, 2 = no CCF condition applied). Exiting...\n",
             doCCFsyst);
      exit(1);
    } else {
      DoCCFsyst = doCCFsyst;
    ...
    if (in_HFMaxRapDefn == 5 || in_HFMaxRapDefn == 5.2) {
      ...
      cout << "[Warning] Using unspecified definition for HFEMax. Max rapidity of HF was likely determined by forest "
              "settings."
           << endl;
      HFMaxRapDefn = in_HFMaxRapDefn;
    } else {
  ...
  int DoCCFsyst;       // Systematic study: CCFs variation
  ...
  void printParameters() const {
    ...
    cout << "TriggerChoice: "
         << ((TriggerChoice == 0)   ? "None"
             ...
             : (TriggerChoice == 4) ? "isL1ZDCXORJet16"
                                    : "")
         << endl;
    cout << "BkgFilterChoice: "
         << ((TriggerChoice == 1)   ? "ClusterCompatibilityFilter + cscTightHalo2015Filter"
    ...
    cout << "DoSystRapGap: "
         << ((DoSystRapGap == 0)   ? "No"
             ...
             : (DoSystRapGap > 9)  ? Form("Custom threshold (%.1f)", ((float)DoSystRapGap) / 10.)
                                   : "Loose")
         << endl;
    cout << "DoSystD: "
         << ((DoSystD == 0)   ? "No"
    ...
    cout << "DoCCFsyst: "
         << ((DoCCFsyst == 0)   ? "Nominal"
             : (DoCCFsyst == 1) ? "Select events failing CCF"
                                : "No CCF condition applied")
         << endl;
...
void saveParametersToHistograms(const Parameters &par, TFile *outf) {
  ...
  TH1D *hDoCCFsyst = new TH1D("parDoCCFsyst", "parDoCCFsyst", 1, 0, 1);
  hDoCCFsyst->SetBinContent(1, par.DoCCFsyst);

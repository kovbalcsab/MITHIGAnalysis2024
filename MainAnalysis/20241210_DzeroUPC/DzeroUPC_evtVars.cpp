...
//============================================================//
bool eventSelection(DzeroUPCTreeMessenger *b, const Parameters &par, TH1D *hEventsPassedSelection) {
...
private:
  ...
  void initHistograms(Parameters &par) {
    ...
    hEventsPassedSelection = new TH1D(Form("hEventsPassedSelection%s", title.c_str()), "Events Passed Selection", 6, -0.5, 5.5);
    ...
    for (int i=0; i<histTypes.size(); i++) {
      ...
      hnTrackInAcceptanceHP.push_back(new TH1D(Form("hnTrackInAcceptanceHP%s%s", title.c_str(), type.c_str()), "", 200, 0, 400));
      ...
      hDNumPassingDcuts.push_back(new TH1D(Form("hDNumPassingDcuts%s%s", title.c_str(), type.c_str()), "", 20, -0.5, 19.5));
  ...
  void fillEvtVarsData(Parameters &par, bool recoDzeroFound, bool hasSignalD, int numPassedD) {
    for (int i=0; i < hHFEmaxMinus.size(); i++) {
      ...
      if (doFill) {
        hHFEmaxMinus[i]->Fill(MDzeroUPC->HFEMaxMinus);
        hHFEmaxPlus[i]->Fill(MDzeroUPC->HFEMaxPlus);
...
int main(int argc, char *argv[]) {
  ...
  int doCCFsyst = CL.GetInt("DoCCFsyst", 0);         // Systematic study: CCFs variation
  ...
  Parameters par(MinDzeroPT, MaxDzeroPT, MinDzeroY, MaxDzeroY, IsGammaN, TriggerChoice, IsData, scaleFactor,
                 ...
                 DoMultReweighting, MultWeightFileName, doCCFsyst);

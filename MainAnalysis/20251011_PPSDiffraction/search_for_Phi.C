...
int main(int argc, char **argv) {
  ...
  TH2D *hMassvsEta =
      new TH2D("hMassvsEta", "Two track Mass vs Eta;Mass (GeV/c^{2});#eta;Entries", 480, 0.8, 2, 100, -2.4, 2.4);
  TH2D *hMassvsPt =
      new TH2D("hMassvsPt", "Two track Mass vs p_{T};Mass (GeV/c^{2});p_{T} (GeV/c);Entries", 480, 0.8, 2, 100, 0, 5);
  TH3D *hMassvsPtvsEta =
      new TH3D("hMassvsPtvsEta", "Two track Mass vs p_{T} vs Eta;Mass (GeV/c^{2});p_{T} (GeV/c);#eta;Entries", 480, 0.8,
               2, 100, 0, 5, 100, -2.4, 2.4);
  ...
  TH1F *hFSC2bottomM_chargefC =
      new TH1F("hFSC2bottomM_chargefC", "FSC2 Bottom Charge;Charge (fC);Entries", 5000, 0, 40000);
  TH1F *hFCS2topM_chargefC = new TH1F("hFCS2topM_chargefC", "FSC2 Top Charge;Charge (fC);Entries", 5000, 0, 40000);
  TH1F *hFSC3bottomleftM_chargefC =
      new TH1F("hFSC3bottomleftM_chargefC", "FSC3 Bottom Left Charge;Charge (fC);Entries", 5000, 0, 40000);
  TH1F *hFSC3bottomrightM_chargefC =
      new TH1F("hFSC3bottomrightM_chargefC", "FSC3 Bottom Right Charge;Charge (fC);Entries", 5000, 0, 40000);
  TH1F *hFSC3topleftM_chargefC =
      new TH1F("hFSC3topleftM_chargefC", "FSC3 Top Left Charge;Charge (fC);Entries", 5000, 0, 40000);
  TH1F *hFSC3toprightM_chargefC =
      new TH1F("hFSC3toprightM_chargefC", "FSC3 Top Right Charge;Charge (fC);Entries", 5000, 0, 40000);

  TH2F *hTrackMomvsdedx =
      new TH2F("hTrackMomvsdedx", "Track Momentum vs dE/dx;Momentum (GeV/c);log(dE/dx (MeV/cm));Entries", 200, 0, 10,
               200, 0, 10);
  TH2F *hTrackMomvsdedxK =
      new TH2F("hTrackMomvsdedxK", "Track Momentum vs dE/dx;Momentum (GeV/c);log(dE/dx (MeV/cm));Entries", 200, 0, 10,
               200, 0, 10);
  TH2F *hTrackMomvsdedxPi =
      new TH2F("hTrackMomvsdedxPi", "Track Momentum vs dE/dx;Momentum (GeV/c);log(dE/dx (MeV/cm));Entries", 200, 0, 10,
               200, 0, 10);
  ...
  float HF_threshold = 16.0; // is this good?
  ...
  for (int i = 0; i < nEntries; ++i) {
    ...
    // Apply baseline event selection (VZ, PVFilter)
    if (!M->PVFilter || fabs(M->VZ) > 15) {
      continue;
    }
    ...
    for (size_t iTrkId = 0; iTrkId < M->trkPt->size(); ++iTrkId) {
      ...
      kaons[iKaon] =
          ROOT::Math::PtEtaPhiMVector(M->trkPt->at(iTrkId), M->trkEta->at(iTrkId), M->trkPhi->at(iTrkId), 0.493677);
    ...
    float PIDScoreKaon0 = GetPIDScore(kaons[0].P(), M->dedxAllLikelihood->at(kaonIdxs[0]), fdedxKaonCenter,
                                      fdedxKaonSigmaLo, fdedxKaonSigmaHi);
    float PIDScoreKaon1 = GetPIDScore(kaons[1].P(), M->dedxAllLikelihood->at(kaonIdxs[1]), fdedxKaonCenter,
                                      fdedxKaonSigmaLo, fdedxKaonSigmaHi);
    float PIDScorePion0 = GetPIDScore(kaons[0].P(), M->dedxAllLikelihood->at(kaonIdxs[0]), fdedxPionCenter,
                                      fdedxPionSigmaLo, fdedxPionSigmaHi);
    float PIDScorePion1 = GetPIDScore(kaons[1].P(), M->dedxAllLikelihood->at(kaonIdxs[1]), fdedxPionCenter,
                                      fdedxPionSigmaLo, fdedxPionSigmaHi);
    float PIDScoreProt0 = GetPIDScore(kaons[0].P(), M->dedxAllLikelihood->at(kaonIdxs[0]), fdedxProtCenter,
                                      fdedxProtSigmaLo, fdedxProtSigmaHi);
    float PIDScoreProt1 = GetPIDScore(kaons[1].P(), M->dedxAllLikelihood->at(kaonIdxs[1]), fdedxProtCenter,
                                      fdedxProtSigmaLo, fdedxProtSigmaHi);

    bool passPID = ApplyPIDCut(kaons[0].P(), PIDScorePion0, PIDScoreKaon0, PIDScoreProt0, kaons[1].P(), PIDScorePion1,
                               PIDScoreKaon1, PIDScoreProt1, true);
    if (!passPID) {
      continue;
    }
    ...
    if (TMath::Abs(diKaon.Eta()) > 2.) {
      continue;
    }

    // Fill histograms for phi meson candidates
    hMass->Fill(diKaon.M());
    hPhiPt->Fill(diKaon.Pt());
    hPhiEta->Fill(diKaon.Eta());
    hPhiPhi->Fill(diKaon.Phi());
    hMassvsEta->Fill(diKaon.M(), diKaon.Eta());
    hMassvsPt->Fill(diKaon.M(), diKaon.Pt());
    hMassvsPtvsEta->Fill(diKaon.M(), diKaon.Pt(), diKaon.Eta());

    if (diKaon.Pt() > 0.4 && 2 > diKaon.Pt()) {
    ...
    hFSC3bottomleftM_chargefC->Fill(M->FSC3bottomleftM_chargefC->at(2));
    hFSC3bottomrightM_chargefC->Fill(M->FSC3bottomrightM_chargefC->at(2));
    hFSC3topleftM_chargefC->Fill(M->FSC3topleftM_chargefC->at(2));
    hFSC3toprightM_chargefC->Fill(M->FSC3toprightM_chargefC->at(2));
    hFSC2bottomM_chargefC->Fill(M->FSC2bottomM_chargefC->at(2));
    hFCS2topM_chargefC->Fill(M->FSC2topM_chargefC->at(2));
    ...
    bool passPPS = (M->PPSStation0M_x->size() > 0 && M->PPSStation2M_x->size() > 0 && M->PPSStation0M_y->size() > 0 &&
                    M->PPSStation2M_y->size() > 0);
  ...
  hHFPlusPPS->Write();
  hZDCPlusPPS->Write();
  hFSC3bottomleftM_chargefC->Write();
  hFSC3bottomrightM_chargefC->Write();
  hFSC3topleftM_chargefC->Write();
  hFSC3toprightM_chargefC->Write();
  hFSC2bottomM_chargefC->Write();
  hFCS2topM_chargefC->Write();
  hTrackPtPPS->Write();
  hTrackEtaPPS->Write();
  hMassPPS->Write();
  hPhiPtPPS->Write();

...
YEAR=2023
ISDATA=false
TAG="Dzero_2023MC_gN-PhotonA_Pthat2"
...
# Output Settings:
OUTPUT="/data00/jdlang/UPCDzeroAnalysis/SkimsMC/output_${TAG}_${DATE}"
MERGEDOUTPUT="/data00/jdlang/UPCDzeroAnalysis/SkimsMC/${TAG}_${DATE}.root"
...
while IFS= read -r FILEPATH; do
    if (( $INPUT_ON_XRD == 1 )); then
        ...
        ./ProcessSkim_Local.sh Execute_Dzero \
            $FILEPATH $COUNTER $OUTPUT $YEAR $ISDATA \
            $TRIG_REJECT $EVENT_REJECT $ZDCGAP_REJECT $D_REJECT \

...
#include "include/LambdaCpksSelection.h"
using namespace LambdaCpksSelection;
...
int main(int argc, char *argv[]) {
  ...
  TTree Tree("Tree", Form("Tree for UPC Lambda_c->p+K_s analysis (%s)", VersionString.c_str()));
  TTree InfoTree("InfoTree", "Information");
  LambdaCpksUPCTreeMessenger MLambdaCUPC;
    ...
    for (int iE = 0; iE < EntryCount; iE++) {
      ...
      if (IsData == false) {
        ...
        for (int iDGen = 0; iDGen < MDfinderGen.Gsize; iDGen++) {
          ...
          bool isFeeddownGen =
              (MDfinderGen.GBAncestorpdgId[iDGen] >= 500 && MDfinderGen.GBAncestorpdgId[iDGen] < 600) ||
...
//        bool selectedVtxFilter = MSkim.PVFilter == 1 && fabs(MTrackPbPbUPC.zVtx->at(0)) < 15.;
      ...
      for (int iD = 0; iD < MLambdaC.Dsize; iD++) {
        ...
        MLambdaCUPC.DRestrk1P->push_back(          MLambdaC.DRestrk1P[iD]);
        MLambdaCUPC.DRestrk1Pt->push_back(         MLambdaC.DRestrk1Pt[iD]);
        MLambdaCUPC.DRestrk1PtErr->push_back(      MLambdaC.DRestrk1PtErr[iD]);
        ...
        if (DoPID && MLambdaC.DRestrk1P[iD] < 2.5) { // Only give valid PID for p_track < 2.5 GeV
          MLambdaCUPC.DRestrk1PionScore->push_back(GetPIDScore(
            ...
            fdedxPionCenter, fdedxPionSigmaLo, fdedxPionSigmaHi));
          MLambdaCUPC.DRestrk1KaonScore->push_back(GetPIDScore(
            MLambdaC.DRestrk1P[iD], MLambdaC.DRestrk1dedx[iD],
            fdedxKaonCenter, fdedxKaonSigmaLo, fdedxKaonSigmaHi));
          MLambdaCUPC.DRestrk1ProtScore->push_back(GetPIDScore(
            MLambdaC.DRestrk1P[iD], MLambdaC.DRestrk1dedx[iD],
            fdedxProtCenter, fdedxProtSigmaLo, fdedxProtSigmaHi));
        } else {
          MLambdaCUPC.DRestrk1PionScore->push_back(-999.);
          MLambdaCUPC.DRestrk1KaonScore->push_back(-999.);
          MLambdaCUPC.DRestrk1ProtScore->push_back(-999.);
        }
        
        MLambdaCUPC.DRestrk2P->push_back(          MLambdaC.DRestrk2P[iD]);
        MLambdaCUPC.DRestrk2Pt->push_back(         MLambdaC.DRestrk2Pt[iD]);
        MLambdaCUPC.DRestrk2PtErr->push_back(      MLambdaC.DRestrk2PtErr[iD]);
        ...
        if (DoPID && MLambdaC.DRestrk2P[iD] < 2.5) { // Only give valid PID for p_track < 2.5 GeV
          MLambdaCUPC.DRestrk2PionScore->push_back(GetPIDScore(
            ...
            fdedxPionCenter, fdedxPionSigmaLo, fdedxPionSigmaHi));
          MLambdaCUPC.DRestrk2KaonScore->push_back(GetPIDScore(
            MLambdaC.DRestrk2P[iD], MLambdaC.DRestrk2dedx[iD],
            fdedxKaonCenter, fdedxKaonSigmaLo, fdedxKaonSigmaHi));
          MLambdaCUPC.DRestrk2ProtScore->push_back(GetPIDScore(
            MLambdaC.DRestrk2P[iD], MLambdaC.DRestrk2dedx[iD],
            fdedxProtCenter, fdedxProtSigmaLo, fdedxProtSigmaHi));
        } else {
          MLambdaCUPC.DRestrk2PionScore->push_back(-999.);
          MLambdaCUPC.DRestrk2KaonScore->push_back(-999.);
          MLambdaCUPC.DRestrk2ProtScore->push_back(-999.);
        }
...
//            (MLambdaC.DgenBAncestorpdgId[iD] >= 500 && MLambdaC.DgenBAncestorpdgId[iD] < 600) ||

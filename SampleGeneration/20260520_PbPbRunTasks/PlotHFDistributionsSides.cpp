...
TH2D BuildPaletteHelper(const std::vector<TH2D *> &Histograms, const std::string &Title, double XMinimum,
                        double XMaximum, double YMinimum, double YMaximum, double FillValue) {
...
void DrawSidePDF(const std::string &OutputPDF, const std::string &CanvasTitle, const DistributionSet &Set,
  ...
  TH2D PaletteFrame =
      BuildPaletteHelper(Histograms, CanvasTitle, XMinimum, XMaximum, YMinimum, YMaximum, Range.Minimum);
  ...
  for (size_t i = 0; i < Histograms.size(); ++i) {
    const double FullMinimum = Histograms[i]->GetMinimum();
    const double FullMaximum = Histograms[i]->GetMaximum();

    Histograms[i]->SetMinimum(Range.Minimum);
    Histograms[i]->SetMaximum(Range.Maximum);
    Histograms[i]->Draw("COL SAME");

    Histograms[i]->SetMinimum(FullMinimum);
    Histograms[i]->SetMaximum(FullMaximum);
    Histograms[i]->Draw("TEXT SAME");
    sideSum += Histograms[i]->Integral();
  }
  TLatex Latex;
  Latex.SetNDC();
  Latex.SetTextAlign(11);
  Latex.SetTextSize(0.035);
  Latex.DrawLatex(0.12, 0.96, Form("Sum: %.4f", sideSum));
...
std::string BuildOutputName(const std::string &OutputDir, const std::string &OutputPrefix, const std::string &ModeLabel,
                            const std::string &IDLabel, const std::string &SideLabel, const std::string &OutputFormat) {
  return OutputDir + "/" + OutputPrefix + "_" + ModeLabel + "_" + IDLabel + "_" + SideLabel + "." + OutputFormat;
}

std::string BuildOutputNameNoID(const std::string &OutputDir, const std::string &OutputPrefix,
                                const std::string &ModeLabel, const std::string &SideLabel,
                                const std::string &OutputFormat) {
  return OutputDir + "/" + OutputPrefix + "_" + ModeLabel + "_" + SideLabel + "." + OutputFormat;
}
} // namespace
...
int main(int argc, char *argv[]) {
  ...
  for (const std::string &ModeLabel : ModeLabels) {
    for (const std::string &IDLabel : IDLabels) {
      ...
      const bool HasPositive =
          LoadSet(InputFile, "Positive", Suffix, HistogramSuffix, PositiveSet, IgnoreEmptyWrapHigh);
      const bool HasNegative =
          LoadSet(InputFile, "Negative", Suffix, HistogramSuffix, NegativeSet, IgnoreEmptyWrapHigh);
  ...
  DistributionSet LeadingProbPositiveSet;
  DistributionSet LeadingProbNegativeSet;
  const bool HasLeadingProbPositive =
      LoadSet(InputFile, "Positive", "_leadingProb", HistogramSuffix, LeadingProbPositiveSet, IgnoreEmptyWrapHigh);
  const bool HasLeadingProbNegative =
      LoadSet(InputFile, "Negative", "_leadingProb", HistogramSuffix, LeadingProbNegativeSet, IgnoreEmptyWrapHigh);

  if (HasLeadingProbPositive == true && HasLeadingProbNegative == true) {
    const std::string PlusPDF = BuildOutputNameNoID(OutputDir, OutputPrefix, "leadingProb", "Plus", OutputFormat);
    const std::string MinusPDF = BuildOutputNameNoID(OutputDir, OutputPrefix, "leadingProb", "Minus", OutputFormat);

    DrawSidePDF(PlusPDF, "HF+ leadingProb", LeadingProbPositiveSet, "leadingProb");
    DrawSidePDF(MinusPDF, "HF- leadingProb", LeadingProbNegativeSet, "leadingProb");
  } else {
    DeleteSet(LeadingProbPositiveSet);
    DeleteSet(LeadingProbNegativeSet);
  }
